...
def write_input(f_instate, rdir='.'):
    ...
    j_rf = instate['j_ec'] + instate['j_ic'] + instate['j_lh']
    pe_rf = instate['pe_ec'] + instate['pe_ic'] + instate['pe_lh']
    pi_rf = instate['pi_ic'] + instate['pe_lh']

    time = 0.

    f = open(os.path.join(rdir, 'inprof'), 'w')
    f.write(' # generated by fastran_io_instate.py\n')
    zfdat.write_f('inflag', [1.], '', f)
    zfdat.write_f('time0', [time * 1.e-3], 's', f)
...
def update_state(f_instate, f_fastran, relax=1.):
    ...
    if nrho != instate['nrho'][0]:
        raise Exception(
            'nrho differ, fastran: %d, instate: %s' %
            (nrho, ps.nrho))
    ...
    instate['p_rad'] = fastran.variables['p_rad'][-1, :]
