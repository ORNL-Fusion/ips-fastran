...
def update_state(
        f_state, f_eqdsk, f_fastran, f_instate, time='', relax=1., relax_J=1., adjust_ip=0, fni_target=1., relax_ip=0.3):
    # -----------------------------------------------------------------
    ...
    for k in range(spec["n_imp"]):
        ps["ns"][spec["k_imp"][k], :] = 1.0e19*ps.node2cell(nz[k])
    if spec["k_he4"] > 0:
        ps["ns"][spec["k_he4"], :] = 1.0e19*ps.node2cell(nhe)
