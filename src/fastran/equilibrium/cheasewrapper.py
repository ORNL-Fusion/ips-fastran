...
def init_chease_inputs(srcVals={},namelistVals={},importedVals={}):
    ...
    elif 'R0EXP' in importedVals:
          R0EXP = importedVals['R0EXP']
    else:
          R0EXP = abs(eqdskdata['RCTR'])

    if   'B0EXP' in namelistVals:
          B0EXP = namelistVals['B0EXP']
    elif 'B0EXP' in importedVals:
          B0EXP = importedVals['B0EXP']
    else:
          B0EXP = abs(eqdskdata['BCTR'])

    if   'ITEXP' in namelistVals:
          ITEXP = namelistVals['ITEXP']
    elif 'ITEXP' in importedVals:
          ITEXP = importedVals['ITEXP']
    else:
    ...
    if   int(namelist['NEQDSK']) == 1:
         ...
         expeqParam['boundary_src']  =  boundary_src
         expeqParam['boundary_type'] =  boundary_type
