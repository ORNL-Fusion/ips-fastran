...
def write_toray_input(geq, prof, intoray):
    ...
    psir = 1.e5 * psir  # cgs unit!

    tmp = psir
    nrho_m = len(tmp)
    rho_m = arange(nrho_m) / (nrho_m - 1.)
    ...
    psin_geq = arange(geq['nw']) / (geq['nw'] - 1.)
    rho_eval = rhopsi(psin_geq)
    rho_eval[0] = 0.
    rho_eval[-1] = 1.

    tmp = zeros(len(rho_eval)) # set hfact = 0, not used by TORAY
    write_formatted(f, tmp, '%16.9e', 5)
...
#   tmp = geq['__gri__']*geq['__R_axis__']
    ...
    r0 = geq['rzero'] * 100.  # [cm]
    bt0 = abs(geq['bcentr']) * 1.e4  # [Gauss]
    ra = 1.e2 * (geq['__phit__'][-1] / pi / abs(geq['bcentr']))**0.5  # [cm]
   ...
   # write_formatted(f,psin_eval,'%16.9e',5)
    ...
    write_formatted(f, prof['ne'] * 1.e13, '%16.9e', 5)  # cm^-3
...
def read_toray_output(f_ncfile='toray.nc'):
    ...
    for i in range(1, nrho - 1):
        jec_out[i] = 0.5 * (jec[i] + jec[i - 1]) * 1.e4
        qec_out[i] = 0.5 * (qec[i] + qec[i - 1]) * 1.e6
...
def cur_integral(rho, jpar, geq, r0, b0):
    ...
    for i in range(1, nrho):
        ...
        ipol_m /= r0 * b0
        I += jpar_m * dvol / ipol_m**2 / (2. * pi * r0)
    return I
...
def fit_gaussian(rho, f):
    def fitfunc(p, x): return p[0] * exp(-(x - p[1])**2 / (2 * p[2]**2))
...
def adjust_jec(width, xmid, fmax, Iec, nrho, geq, r0, b0):
    def gauss(p, x): return p[0] * exp(-(x - p[1])**2 / (2 * p[2]**2))

    def func(f):
        x = arange(nrho) / (nrho - 1.)
        y = gauss([f, xmid, width], x)
        return cur_integral(x, y, geq, r0, abs(b0)) - Iec

    tmp = optimize.bisect(func, 0., 10 * fmax, xtol=1.e-6)
...
def gauss_asym(p, x):
    ...
    for k in range(len(x)):
        if x[k] >= p[1]:
            rvec[k] = p[0] * exp(-(x[k] - p[1])**2 / (2 * p[2]**2))
        if x[k] < p[1]:
            rvec[k] = p[0] * exp(-(x[k] - p[1])**2 / (2 * p[3]**2))
    return rvec
...
def update_state(geq, ps, intoray):
    ...
    for k in range(ntoray):
        rfpow = intoray['intoray']['rfpow'][k] * 1.e-6  # [MW]
    ...
    qec_sum[0] = qec_sum[1]
    rho_ps = ps['rho'][:]
    jec_ps = 1.e6 * interp1d(rho, jec_sum, kind='cubic')(rho_ps)
    qec_ps = 1.e6 * interp1d(rho, qec_sum, kind='cubic')(rho_ps)
