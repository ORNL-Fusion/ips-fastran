...
class Timetrace():
    def __init__(self, fname, kind='linear'):
        ...
        for key in ['ip', 'bt', 'r0']:
            ...
            f = nc.variables[key][:]
            self.data[key] = f
            self.interpolation[key] = interp1d(
                t, f, kind=kind, fill_value=(f[0], f[-1],), bounds_error=False)
        ...
        for key in var_profile + var_nb + var_ext:
            if key in nc.variables.keys():
                ...
                self.data[key] = f
                self.interpolation[key] = interp2d(
                    t, x, f.transpose(), kind=kind)
        ...
        for key in var_nbi:
            if key in list(nc.variables.keys()):
                ...
                for k in range(len(self.data[key])):
                    ...
                    self.interpolation[key][k] = interp1d(
                        t, f, kind=kind, fill_value=(f[0], f[-1],), bounds_error=False)

        # ec power
        var_ech = ['pech']  # ['pech', 'aziang', 'polang']
        for key in var_ech:
            if key in list(nc.variables.keys()):
                self.data[key] = nc.variables[key][:, :]
                self.interpolation[key] = len(self.data[key]) * [None]
                for k in range(len(self.data[key])):
                    t = nc.variables['time_ech'][:]
                    f = nc.variables[key][k, :]
                    self.interpolation[key][k] = interp1d(
                        t, f, kind=kind, fill_value=(f[0], f[-1],), bounds_error=False)
    ...
    def slice_list(self, key, t):
        return [self.interpolation[key][k](t) 
            for k in range(len(self.interpolation[key]))]
