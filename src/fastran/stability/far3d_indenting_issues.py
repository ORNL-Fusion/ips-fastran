...
class far3d(Component):
    ...
    def step(self, timeid=0):
        ...
        for n in n_values:
            ...
            # call the helper
            self.run_far3d_with_dask(run_dirs, far3d_bin, dask_nodes)
    
        # Step 2: Launch FAR3d in each folder (no wait)
   #     for d in run_dirs:
   #         self.services.launch_task(1, str(d), far3d_bin, logfile="xfar3d.log")
    def run_far3d_with_dask(self, run_dirs, far3d_bin, dask_nodes):
    ...
    for i, rdir in enumerate(run_dirs, start=1):
        ...
        logfile = os.path.join(str(rdir), f"xfar3d_{i:02d}.log")
            ...
            # If you want to ensure the exe is visible in each dir, either copy it
            # beforehand or rely on PATH. Here we just run it from PATH / absolute path.
        self.services.add_task(
            pool_name,
            f"far3d_task_{i:02d}",      # task label
            1,                          # processes per task
            str(rdir),                  # working directory
            far3d_bin,                  # executable (absolute path or on PATH)
            logfile=logfile             # capture stdout/err
            # , env=my_env_dict         # optional: per-task environment
        )

        # submit the whole pool to Dask workers
        # IPS will fan out tasks to Dask; each task runs in its own working dir
        ret_val = self.services.submit_tasks(
            pool_name,
            use_dask=True,
            dask_nodes=1         # how many Dask workers/nodes to use
        )
        ...
        if failed:
            # surface which directories failed
