...
#        print('Running vmeclauncher')
        ...
        betalf = 2.0 * mu0 * peak_nalpha*1e20*peak_talpha*e_charge*1e3 / (B0_e**2)
        va0 = B0_e / math.sqrt(uion_e * mu0 * peak_nion * m_p)
        ...
        for n in n_values:
            ...
            customized = (
                template.replace("<MM_LINE>", format_list("mm", mm_vals))
                        ...
                        .replace("<OMCY>",format_scalar("omcy"),omcy)
                        .replace("<OMCYA>",format_scalar("omcyalp"),omcya)
                        )
    ...
    def finalize(self, timeid=0):
        ...
        istate['EP']['growth_rate']=growth_rate
        istate['EP']['frequency']=frequency
