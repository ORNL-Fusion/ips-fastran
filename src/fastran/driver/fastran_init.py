...
class fastran_init (Component):
    ...
    def step(self, timeid=0):
        ...
        if input_dir_id:
            self.services.component_ref.config['INPUT_DIR'] = \
                self.services.component_ref.config['INPUT_DIR'] + '_%d' % int(float(input_dir_id))
        ...
        # -- recycling neutral
        neutral_energy = namelist_default(
            instate.data, 'instate', 'neutral_energy', [15.])[0]
        neutral_recycling = namelist_default(
            instate.data, 'instate', 'neutral_recycling', [1.e20])[0]
        ...
        ps.variables['e0_av'][:] = [neutral_energy * 1.e-3]
        ps.close()
        ...
        # -- eped
        if f_eped:
            cesol_io.eped_to_plasmastate(
                cur_state_file, f_eped, cur_instate_file)
