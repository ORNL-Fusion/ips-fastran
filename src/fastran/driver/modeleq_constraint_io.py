...
from fastran.util.model_profile import profile_pedestal, profile_hat, profile_spline
...
def calculate_tau98(instate):
    ...
    pe_ec  = vol_integrate(vol, instate['pe_ec'])
    pe_ic  = vol_integrate(vol, instate['pe_ic'])
    pi_ic  = vol_integrate(vol, instate['pi_ic'])
    pe_nb  = vol_integrate(vol, instate['pe_nb'])
    pi_nb  = vol_integrate(vol, instate['pi_nb'])
    pe_fus = vol_integrate(vol, instate['pe_fus'])
    pi_fus = vol_integrate(vol, instate['pi_fus'])
    p_rad = vol_integrate(vol, instate['p_rad'])
    ploss = pe_ec + pe_ic + pi_ic + pe_nb + pi_nb + pe_fus + pi_fus + p_rad
...
def update_state(f_instate, nmax_iter=100):
    ...
    # -------------------------------------------------------------------
    # calculate betan
    wth = 1.5 * 1.602e3 * (instate['ne']*instate['te'] + (instate['ni'] + instate['nz'])*instate['ti']) # Joul
    ...
    # _wth = np.sum([ (vol[i + 1] - vol[i]) * wth[i] for i in range(nrho - 1) ])
    _wth = np.sum([ (vol[i + 1] - vol[i]) * 0.5 * (wth[i+1] + wth[i]) for i in range(nrho - 1) ])
    ...
    if h98_target > 0:
        tau98, pinj = calculate_tau98(instate)
        wth_target = tau98 * h98_target * pinj * 1.e6
        mu0 = 4.e-7 * np.pi
        betan_th_target = 4. / 3. * mu0 * a0 / (ip * b0) * wth_target / vol[-1] * 100.
        ...
        print(f'wth_target = {1.e-6*wth_target}, wth = {1.e-6*_wth}')
        print(f'tauth = {1.e-6*_wth/pinj}, h98 = {1.e-6*_wth/pinj/tau98}')
    else:
    ...
    instate['betan_th_target'] = [betan_th_target]
    instate['betan_beam_target'] = [betan_beam_target]
    ...
    if betan_beam_target > 0.:
        density_beam = (instate['nbeam_axis'][0] - instate['nbeam_sep'][0]) \
            * (1. - instate['rho']**instate['nbeam_alpha'][0])**instate['nbeam_beta'][0] \
            + instate['nbeam_sep'][0]
        wbeam = 1.5 * 1.602e3 * density_beam * instate['tbeami'][0]
    ...
    if betan_th_target > 0.:
        ...
        for iter_scale in range(nmax_iter):
            ...
            print('scale to match betan_th :')
            print(f'tscale = {tscale:6.3e}, betan_th = {betan_th:6.3e}, betan_th_target = {betan_th_target:6.3e}')
    ...
    if pressure_model in ['kinetic', 'full']:
        ...
        p_axis_min = 0.
        p_axis_max = 100. * instate['ptot_axis'][0]
        ...
        for iter_scale in range(nmax_iter):
            ...
            print('scale to match betan : %6.3e, %6.3e' % (p_axis*1.e-3, betan))
