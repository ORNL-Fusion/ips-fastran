...
# def limit_betaN(f_state, f_eqdsk):
#
...
#
#     pmhd = 1.5*1.602e3*(ne*te+ni*ti)+1.602e3*(wbeam+walpha)
#
#     mu0 = 4.0*pi*1.0e-7
...
def parabolicJ(f_state, f_eqdsk, f_bc, rho_jbdry=0.8, q0=1.1,alpha=3.0):
...
def broadJ(f_state, f_eqdsk, rho_j, j_in, j_out,rho_jbdry):
    ...
    for iter_scale in range(100):
        ...
        j_tot = zeros(nrho)
        for i in range(nrho):
            if rho[i]<rho_jbdry: j_tot[i] = gauss_asym([x,rho_j,j_out,j_in],[rho[i]])[0] - gauss_asym([x,rho_j,j_out,j_in],[rho_jbdry])[0] + jbdry
           #if rho[i]<rho_jbdry: j_tot[i] = gauss_asym([x,0.65,0.1,0.5],[rho[i]])[0] - gauss_asym([x,0.65,0.1,0.5],[rho_jbdry])[0] + jbdry
           #if rho[i]<rho_jbdry: j_tot[i] = gauss_asym([x,0.65,0.1,0.6],[rho[i]])[0] - gauss_asym([x,0.65,0.1,0.6],[rho_jbdry])[0] + jbdry
           #if rho[i]<rho_jbdry: j_tot[i] = gauss_asym([x,0.65,0.1,0.8],[rho[i]])[0] - gauss_asym([x,0.65,0.1,0.8],[rho_jbdry])[0] + jbdry
           #if rho[i]<rho_jbdry: j_tot[i] = gauss_asym([x,0.65,0.1,0.4],[rho[i]])[0] - gauss_asym([x,0.65,0.1,0.4],[rho_jbdry])[0] + jbdry
            else: j_tot[i] = j_bs[i]
...
def broadJ_spline(f_state, f_eqdsk, rho_j, jaxis, rho_jbdry):
