...
module pstool_io_mod
  ...
  real(kind=rspec), parameter :: mu0   = 4.0*3.1415926535897931d-7
  ...
  integer :: nspec_fusion = 1
  integer :: nspec_rfmin = 0
  integer :: nspec_neut = 1
  ...
  real*8 difb_a,difb_0,difb_in,difb_out
  real*8 dn0out
  ...
  real(kind=rspec) :: del_box   = 0.5    ! user input, later
  real(kind=rspec) :: del_lim   = 0.1    ! user input, later

  !--eqdat

  integer :: nrho_inv,nth_inv,nrho_inv_geo

  real*8, dimension(:), allocatable :: th_inv,rho_inv
  real*8, dimension(:), allocatable :: psi_inv,phi_inv,g_inv
  real*8, dimension(:,:), allocatable :: r_inv,z_inv
  real*8, dimension(:), allocatable :: p_inv,q_inv
  real*8, dimension(:), allocatable :: press_inv,jpar_inv

  real*8 :: ip_inv,r0_inv,b0_inv,tflux_inv

  !-- default file names

  character*128 :: file_geqdsk = ''
  character*128 :: file_instate = 'instate'
  character*128 :: file_inps = 'inps'
  character*128 :: file_innubeam = 'innubeam'

  !--lun

  integer, parameter :: lun_out = 101
  integer, parameter :: lun_nml = 102
  ...
  namelist /inps/                                 &
     nrho,nrho_eq,nth_eq,nrho_eq_geo,             &
     file_geqdsk,                                 &
     global_label, tokamak_id,                    &
     runid, shot_number,                          &
     time,bdy_crat
...
!=======================================================================
! add genray variables to the statefile
!
subroutine pstool_init_genray()
    ...
    integer :: ierr,i
    integer :: nrho_ecrf = 51

    write(*,*) 'DLG : Running pstool_init_genray()'
...
subroutine pstool_init()
  ...
  integer :: nrho_nbi     = 51
  integer :: nrho_ecrf    = 51
  integer :: nrho_icrf    = 51
  integer :: nrho_fus     = 51
  integer :: nrho_anom    = 51

! integer :: num_rzlim    = 10
...
!   num_rzlim
  ...
  open(unit=lun,file='inps',status='old',iostat=ierr)
  if(ierr.ne.0) then
  ...
  ps%nspec_th = nspec_ion + nspec_imp  ! main ion + impurity
  ...
  do i = 1, nspec_imp
    call ps_species_convert(z_imp(i), z_imp(i), a_imp(i), &
         ps%qatom_s(nspec_ion+i), ps%q_s(nspec_ion+i), ps%m_s(nspec_ion+i), ierr)
    call ckerr('ps_species_convert',ierr)
  enddo
  ...
  enddo
1 format(' Specie index & type: ',i2,1x,i2,1x, &
...
!=======================================================================
! generate geqdsk plasma state
!
subroutine pstool_geqdsk(bdy_crat)
...
subroutine pstool_load_geqdsk(bdy_crat)
